...
class TestIntegrationSubcommand(unittest.TestCase):
    ...
    def test_integration_list(self):
        ...
        self.assertIn(INTEGRATION_GUID, result.output)
        self.assertIn("My OAuth Integration", result.output)
    ...
    def test_integration_show(self):
        ...
        args = apply_common_args(
            ["integration", "show", INTEGRATION_GUID], server=self.connect_server, key=self.api_key
        )
        result = runner.invoke(cli, args)
        self.assertEqual(result.exit_code, 0, result.output)
    ...
    def test_integration_edit_name_only(self):
        ...
        runner = CliRunner()
        args = apply_common_args(
            ["integration", "edit", INTEGRATION_GUID, "-N", "New Name"],
            server=self.connect_server,
            key=self.api_key,
        )
        result = runner.invoke(cli, args)
    ...
    def test_integration_remove(self):
        ...
        runner = CliRunner()
        args = apply_common_args(
            ["integration", "remove", INTEGRATION_GUID], server=self.connect_server, key=self.api_key
        )
        result = runner.invoke(cli, args)
        self.assertEqual(result.exit_code, 0, result.output)
    ...
    def test_templates_show(self):
        ...
        runner = CliRunner()
        args = apply_common_args(
            ["integration", "templates", "show", "custom"], server=self.connect_server, key=self.api_key
        )
        result = runner.invoke(cli, args)
        self.assertEqual(result.exit_code, 0, result.output)
        output = json.loads(result.output)
