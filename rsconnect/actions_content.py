...
def build_add_content(
    ...
    if content_build_store().get_build_running():
    ...
    with RSConnectClient(connect_server) as client:
        ...
        for content in content_to_add.values():
            ...
            content_build_store().add_content_item(content)
            content_build_store().set_content_item_build_status(content["guid"], BuildStatus.NEEDS_BUILD)
...
def build_remove_content(
    ...
    guids: list[str] = [guid]
    if all:
        guids = [c["guid"] for c in content_build_store().get_content_items()]
    for guid in guids:
        content_build_store().remove_content_item(guid, purge)
    return guids
...
def build_list_content(connect_server: RSConnectServer, guid: str, status: Optional[str]):
    init_content_build_store(connect_server)
    if guid:
        return [content_build_store().get_content_item(g) for g in guid]
    else:
        return content_build_store().get_content_items(status=status)


def build_history(connect_server: RSConnectServer, guid: str):
    init_content_build_store(connect_server)
    return content_build_store().get_build_history(guid)
...
def build_start(
    ...
    if all:
        ...
        all_content = content_build_store().get_content_items()
        ...
        if aborted:
            ...
            aborted_content = content_build_store().get_content_items(status=BuildStatus.ABORTED)
        ...
        if error:
            ...
            error_content = content_build_store().get_content_items(status=BuildStatus.ERROR)
        ...
        if running:
            ...
            running_content = content_build_store().get_content_items(status=BuildStatus.RUNNING)
    ...
    content_items = content_build_store().get_content_items(status=BuildStatus.NEEDS_BUILD)
    ...
    try:
        ...
        content_build_store().set_build_running(True)
        ...
        for future in as_completed(build_result_futures):
            ...
            try:
                ...
                content_build_store().set_content_item_build_status(guid_with_bundle.guid, BuildStatus.ERROR)
        ...
        content_build_store().set_build_running(False)
...
def _monitor_build(connect_server: RSConnectServer, content_items: list[ContentItemWithBuildState]):
    ...
    error = []
    start = datetime.now()
    while content_build_store().get_build_running() and not content_build_store().aborted():
        time.sleep(5)
    ...
    if content_build_store().aborted():
        ...
        if len(aborted_builds) > 0:
            ...
            for guid in aborted_builds:
                ...
                content_build_store().set_content_item_build_status(guid, BuildStatus.ABORTED)
        return False
...
def _build_content_item(connect_server: RSConnectServer, content: ContentItemWithBuildState, poll_wait: str):
    ...
    with RSConnectClient(connect_server) as client:
        ...
        content_build_store().update_content_item_last_build_time(guid)
        content_build_store().set_content_item_build_status(guid, BuildStatus.RUNNING)
        content_build_store().ensure_logs_dir(guid)
        try:
            ...
            content_build_store().update_content_item_last_build_log(guid, None)
            raise
        log_file = content_build_store().get_build_log(guid, task_id)
        ...
        content_build_store().update_content_item_last_build_log(guid, log_file)

        if content_build_store().aborted():
            return

        content_build_store().set_content_item_last_build_task_result(guid, task_status)
        if task_status["code"] != 0:
            logger.error("Build failed: %s" % guid)
            content_build_store().set_content_item_build_status(guid, BuildStatus.ERROR)
        else:
            logger.info("Build succeeded: %s" % guid)
            content_build_store().set_content_item_build_status(guid, BuildStatus.COMPLETE)
