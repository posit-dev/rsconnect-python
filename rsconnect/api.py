...
class CloudService:
    ...
    def try_parse_cloud_identifier(identifier: str):
        if isinstance(identifier, str) and identifier.startswith("lucid:content:"):
    ...
    def prepare_deploy(
        ...
        if app_id is None:
            ...
            if cloud_resource_identity.content_id:
                ...
                application = self._rstudio_client.get_application(app_id)
                output = self._rstudio_client.get_content(application["output_id"])
                app_id_int = application["id"]

            if application_type == "static":
                revision = self._rstudio_client.create_revision(output["id"])
